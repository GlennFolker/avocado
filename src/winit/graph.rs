use crate::{
    ...
    winit::{
        FrameBuffer,
        SamplerDesc,
        SurfaceConfig, ClearColor, Frame, Renderer,
        ColorExt as _,
    },
};
use async_channel::{
...
struct OutputChannel {
    sender: Sender<HashMap<SystemLabelId, &'static RenderOutput>>,
    receiver: Receiver<HashMap<SystemLabelId, &'static RenderOutput>>,
}
...
impl RenderState {
    fn new(renderer: &Renderer, surface: &SurfaceConfig) -> Self {
        ...
        let screen = renderer.device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        ...
        let shader = renderer.device.create_shader_module(wgpu::include_wgsl!("screen_quad.wgsl"));
...

        {
            let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                ...
                color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                    ...
                    resolve_target: None,
                    ops: wgpu::Operations {
                        load: wgpu::LoadOp::Clear(clear_color.into_wgpu()),
                        store: true,
                    },
                })],
...
impl<Desc> RenderNode<Desc> where
    ...
    <<Desc as RenderNodeDesc>::Param as SystemParam>::Fetch: ReadOnlySystemParamFetch,
{
    pub fn new(label: SystemLabelId, renderer: &Renderer, done_sender: Sender<usize>) -> Self {
        Self {
            label,
